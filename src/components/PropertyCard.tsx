...
export default function PropertyCard({ prop, index = 0, styleClass = '' }: { prop: any, index?: number, styleClass?: string }) {
  ...
  return (
    <Link href={`/properties/${prop.slug || prop.id}`} className={`${styles.cardLink} ${styleClass} reveal stagger-${index + 1}`}>
      <div className={styles.propertyCard}>
        <div className={styles.imageContainer}>
          ...
          <div className={styles.badge}>{prop.property_type || 'Land Plot'}</div>
        ...
        <div className={styles.propertyInfo}>
          ...
          <p className={styles.propertyLocation}>📍 {prop.village ? `${prop.village}, ` : ''}{prop.taluka || 'Sindhudurg'}</p>
          
          <div className={styles.propertyFeatures}>
            <span>{prop.area_display || `${prop.area || ''} ${prop.area_unit || 'Acres'}`}</span>
            {prop.status && <span className={styles.statusBadge}>{prop.status}</span>}
          </div>

          <div className={styles.priceRow}>
            <div className={styles.price}>{prop.price_display || `₹ ${prop.price}`}</div>
          ...
          <div className={styles.actionButtons}>
            <button 
              onClick={(e) => { 
                e.preventDefault(); 
                e.stopPropagation(); 
                window.location.href = `tel:+918788818163`; 
              }}
